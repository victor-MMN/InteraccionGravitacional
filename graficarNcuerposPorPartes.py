...
def nBodies_Animation3D(num,fps,time,Bodies,data,nameBodies,colorBodies):
    ...
    while j < (Bodies): # While para plotear las trayectorias de las diferentes masas.
        ...
        ax.plot3D(rg[:(fps * num)+1, 0], rg[:(fps * num)+1, 1], rg[:(fps * num)+1, 2], \
                        linewidth=4, color = colorBodies[j])
        ...
        ax.scatter(rg[(fps * num), 0], rg[(fps * num), 1], rg[(fps * num), 2], \
                    marker='o', s=60, color = colorBodies[j])
    ...
    # los limites de z serian de (0,0) lo cual no se pudiera graficar.
    ...
    ax.set_title('Tiempo = ' + str(format(time[fps * num],".4g")) + ' D', fontsize = 30)
    # ax.set_title('Trayectorias')
    ax.set_xlabel(r'$x$ $(M_l)$', fontsize = 15)
    ax.set_ylabel(r'$y$ $(M_l)$', fontsize = 15)
    ax.set_zlabel(r'$z$ $(M_l)$', fontsize = 15)
...
nameFile = "STL_RK4_dt=0.00712890625_N=512000_LunaExagerada" 
N = 512000
totalParts = 1
...
# folder = "tesis\SMVTLM"
# # nameFile = "STLN_RK4_dt=0.00712890625_N=512000"
# nameFile = "TODOS_RK4_dt=0.00712890625_N=512000"
# totalParts = 1
...
fig = plt.figure(figsize=(6, 6), dpi = 120) # figsize : pulgada por pulgada.
...
# data = dd.read_csv(f"datos\{folder}\POS_{nameFile}_P*.txt",sep='\s+', \
#                   header=None, skiprows = 1, dtype = 'float')        
# data = data.compute()        
# data = pd.DataFrame(data)
# data = data.loc[:, 4:] # loc crea un dataframe del existente.
